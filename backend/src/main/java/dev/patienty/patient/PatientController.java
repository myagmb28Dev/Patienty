...
public class PatientController { private final PatientQueryService service; public PatientController(PatientQueryService service){this.service=service;}
    ...
    @GetMapping("/{patientId}") public PatientDtos.PatientDetail detail(@PathVariable UUID patientId){return service.detail(patientId);}
    @GetMapping("/{patientId}/timeline") public List<PatientDtos.TimelineItem> timeline(@PathVariable UUID patientId){return service.timeline(patientId);}
    @GetMapping("/{patientId}/measurements") public List<PatientDtos.MeasurementSeries> measurements(@PathVariable UUID patientId,@RequestParam(defaultValue="")String metric,@RequestParam(required=false)@DateTimeFormat(iso=DateTimeFormat.ISO.DATE_TIME)Instant from,@RequestParam(required=false)@DateTimeFormat(iso=DateTimeFormat.ISO.DATE_TIME)Instant to){return service.measurements(patientId,metric,from,to);}
}
